function [D, H,obj, gradient]=wasserstein_DL_dictionary_step(X,K,lambda,gamma,H,options,isHist)
%% Dictionary step of Wasserstein Dictionary Learning
% This function solves 
%
%               min_{lambda} W_gamma(X,D*lambda)
%
% Inputs:
...
% Projection operator
pinvLambda=pinv(lambda);

    function [projected, gradNorm]=projection(gradient)
        gradNorm=norm(gradient,'fro');
        if gradNorm>0
            projected=(gradient-(gradient*pinvLambda)*lambda)/gradNorm;
            gradNorm=sqrt(sum(sum(projected.*gradient)));
        else
            projected=gradient;
        end
...
[H, obj, gradient]=linear_projected_gradient_descent(H,@(G)computeObj(G),@(G)projection(G),options.dual_descent_stop,options.t0,options.alpha,options.beta,options.verbose);
obj=-obj;
